...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      let q = `SELECT s.*, c.name AS customer_display
               FROM sales s
    ...
    if (req.method === 'DELETE') {
      ...
      await pool.query(`DELETE FROM sale_items WHERE sale_id=?`, [req.query.id]);
    ...
    if (req.method === 'POST') {
      ...
      for (const item of items) {
        if (item.item_id) {
          const [[st]] = await pool.query(`SELECT current_stock, name FROM items WHERE id=?`, [item.item_id]);
      ...
      // Ledger entry if customer selected
      if (customer_id) {
        const [[c]] = await pool.query(`SELECT balance FROM customers WHERE id=?`, [customer_id]);
        const newBal = (parseFloat(c.balance) || 0) + (total - paid);
        await pool.query(`UPDATE customers SET balance=? WHERE id=?`, [newBal, customer_id]);
        await pool.query(
