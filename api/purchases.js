      ...
      try {
        ...
        for (const item of items) {
          await conn.query(`INSERT INTO purchase_items (purchase_id,item_id,item_name,qty,unit_name,rate) VALUES (?,?,?,?,?,?)`,
            [pr.insertId, item.item_id||null, item.item_name, item.qty, item.unit_name||'', item.rate]);
          if (item.item_id) await conn.query(`UPDATE items SET current_stock=current_stock+? WHERE id=?`, [item.qty, item.item_id]);
        }
        if (supplier_id) {
          const [[s]] = await conn.query(`SELECT balance FROM suppliers WHERE id=?`, [supplier_id]);
          ...
          await conn.query(`INSERT INTO ledger_entries (entry_date,party_type,party_id,entry_type,reference_id,debit,credit,balance,notes) VALUES (?,?,?,?,?,?,?,?,?)`,
            [purchase_date,'supplier',supplier_id,'purchase',pr.insertId,paid,total,nb,notes||null]);
        }
        await conn.commit();
